...
export function renderOneOffTab(locale, cashflow) {
  ...
  const items = (cashflow?.oneOffs || []).map(o => {
    ...
    return {
      ...
      name: o.name || o.description || "One-off",
      type: o.type || "expense",
    ...
    <div class="cashflow-tab-content" data-cashflow-oneoff-tab>
      <div class="panel">
        <div class="cashflow-panel-heading">
          <h3>${escapeHtml(t(locale, "Jednorazowe", "One-off"))}</h3>
      <div class="cashflow-tab-actions">
        <button class="btn-primary" data-cashflow-add-oneoff>${escapeHtml(t(locale, "Dodaj transakcję", "Add transaction"))}</button>
      </div>
        </div>

      ${renderDetailsPanel(
        t(locale, "Przyszłe", "Future"),
