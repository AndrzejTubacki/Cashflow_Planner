...
export function renderLedgerTab(locale, cashflow) {
  const pending = (cashflow?.pendingTransactions || []).map(p => ({
    ...p,
    entityType: "pending",
    name: p.name || p.description || p.title || "Pending",
  ...
  const future = (cashflow?.futureTransactions || []).map(f => ({
    ...f,
    entityType: "future",
    name: f.name || f.description || f.title || "Future",
  ...
  const confirmed = (cashflow?.confirmedTransactions || cashflow?.confirmed || []).map(c => ({
    ...
    name: c.name || c.description || c.title || "Confirmed",
  ...
  const confirmedHtml = confirmed.length
    ...
    : `<p>${escapeHtml(t(locale, "No confirmed transactions in API response", "No confirmed transactions in API response"))}</p>`;
  ...
  const futureHtml = future.length
    ...
    : `<p>${escapeHtml(t(locale, "No future transactions", "No future transactions"))}</p>`;
  ...
  return `
    <div class="cashflow-tab-content" data-cashflow-ledger-tab>
      ...
      <div class="panel">
        <h3>${escapeHtml(t(locale, "Pending", "Pending"))}</h3>
      ...
      <div class="panel">
        <h3>${escapeHtml(t(locale, "Confirmed", "Confirmed"))}</h3>
        ${confirmedHtml}
      </div>

      <div class="panel">
        <h3>${escapeHtml(t(locale, "Future", "Future"))}</h3>
        ${futureHtml}
      </div>
    </div>
  `;
